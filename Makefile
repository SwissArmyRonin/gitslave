...
release: README
	@VERSION=`git describe --exact-match --match 'v[0-9]*' 2>/dev/null | \
	  ...
	  if [ $$? -ne 0 -o "$$VERSION" = "" ]; then			     \
	    echo "Not a tagged version, you may not release"; exit 3;	     \
	  fi;								     \
	  if ! git status --porcelain 2>/dev/null; then			     \
	    echo "Git version 1.7 or later required to release"; exit 4;     \
	  fi;								     \
	  if [ `git status --porcelain | wc -l` -gt 0 ]; then		     \
	    echo "Uncommitted changes, you may not release"; exit 2;	     \
	  fi;								     \
	  git checkout-index -a -f --prefix=/tmp/gitslave-$$VERSION/;	     \
	  cd /tmp/gitslave-$$VERSION;					     \
	  sed -i "s/{UNTAGGED}/$${VERSION}/" gits;			     \
	  sed "s/{UNTAGGED}/$${VERSION}/" contrib/gitslave.spec.in	     \
	    > contrib/gitslave.spec;					     \
	  cd ..;							     \
